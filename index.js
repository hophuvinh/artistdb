...
function parsePrefixTags(text, prefix) {
  const re = new RegExp(`${prefix}:\\s*([^msnr:]+?)(?=\\s+[msnr]:|$)`, "i");
...
function parseRemind(text) {
  const match = text.match(/r:\s*(.+?)(?=\s+[msnr]:|$)/i);
  return match ? match[1].trim() : null;
}

// ── /start & /help ────────────────────────────────────────────────────────
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(msg.chat.id, msgHelp(NOTION_DB_URL), {
...
bot.on("message", async (msg) => {
  ...
  if (s.state === "ask_medium") {
    ...
    if (typed.length) s.draft.mediums = mergeTags(s.draft.mediums, typed);
  ...
  if (s.state === "ask_style") {
    ...
    if (typed.length) s.draft.styles = mergeTags(s.draft.styles, typed);
    s.state = "confirm";
    bot.sendMessage(chatId, msgConfirm(s.draft), { reply_markup: kbConfirm() });
    return;
  }
  ...
  try {
    ...
    // Parse remind from raw text if AI missed it
    if (!parsed.remind) {
      parsed.remind = parseRemind(text);
    }
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "confirm:edit_medium") {
    s.state = "ask_medium";
    bot.sendMessage(chatId, msgAskMedium(), { reply_markup: kbMedium(s.draft?.mediums || []) });
    return;
  }
  ...
  if (data.startsWith("medium:")) {
    ...
    if (s.state === "edit_medium") {
      ...
      if (idx >= 0) s.editMediums.splice(idx, 1); else s.editMediums.push(val);
      bot.editMessageReplyMarkup(kbMedium(s.editMediums), { chat_id: chatId, message_id: query.message.message_id });
    } else {
      s.draft = s.draft || {};
      s.draft.mediums = s.draft.mediums || [];
      const idx = s.draft.mediums.map((m) => m.toLowerCase()).indexOf(val);
      if (idx >= 0) s.draft.mediums.splice(idx, 1); else s.draft.mediums.push(val);
      bot.editMessageReplyMarkup(kbMedium(s.draft.mediums), { chat_id: chatId, message_id: query.message.message_id });
    }
    return;
  }
  ...
  if (data.startsWith("style:")) {
    ...
    if (s.state === "edit_style") {
      ...
      if (idx >= 0) s.editStyles.splice(idx, 1); else s.editStyles.push(val);
      bot.editMessageReplyMarkup(kbStyle(s.editStyles), { chat_id: chatId, message_id: query.message.message_id });
    } else {
      s.draft = s.draft || {};
      s.draft.styles = s.draft.styles || [];
      const idx = s.draft.styles.map((m) => m.toLowerCase()).indexOf(val);
      if (idx >= 0) s.draft.styles.splice(idx, 1); else s.draft.styles.push(val);
      bot.editMessageReplyMarkup(kbStyle(s.draft.styles), { chat_id: chatId, message_id: query.message.message_id });
    }
    return;
  }
  ...
  if (data === "edit:medium") {
    ...
    bot.sendMessage(chatId, msgEditMedium(s.artist?.mediums), { reply_markup: kbMedium(s.artist?.mediums || []) });
    return;
  }
  if (data === "edit:style") {
    s.state = "edit_style";
    s.editStyles = [...(s.artist?.styles || [])];
    bot.sendMessage(chatId, msgEditStyle(s.artist?.styles), { reply_markup: kbStyle(s.artist?.styles || []) });
    return;
  }
  if (data === "edit:name") {
    s.state = "edit_name";
  ...
  if (data.startsWith("find_edit:")) {
    const pageId = data.replace("find_edit:", "");
    try {
      const all = await getAllArtists();
